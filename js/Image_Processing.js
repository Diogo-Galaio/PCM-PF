    ...
    count_Pixels (pixels) {
        ...
        for(let i=0; i<pixels.data.length; i+=4){
            for(let j=0; j<this.redColor.length; j++){
                let redD = Math.abs(pixels.data[i]-this.redColor[j]);
                let greenD = Math.abs(pixels.data[i+1]-this.greenColor[j]);
                let blueD = Math.abs(pixels.data[i+2]-this.blueColor[j]);
                let d = redD+greenD+blueD;
                if(d<160 && redD<70 && greenD<70 && blueD<70){
                    hist[j]++;
                }
            }
        }
